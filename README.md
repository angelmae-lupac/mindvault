...
```mermaid
flowchart TB
    subgraph Adding["Adding a document"]
        A[Document<br/>docx, pdf, or txt] --> B[Extract text<br/>mammoth, pdf-parse]
        B --> C[Chunk and embed<br/>800-char chunks to vectors]
        C --> D[(Vector store<br/>Postgres + pgvector)]
    end
    subgraph Asking["Asking a question"]
        E[Question<br/>typed by the user] --> F[Embed question<br/>same embedding model]
        F --> G[Similarity search<br/>top 3 nearest chunks]
        G --> H[Answer<br/>generated by Llama 3.2]
    end
    D -.-> G
```
